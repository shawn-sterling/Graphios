...
def parser_enhanced(raw):
    ...
    #log.debug("CHUNKS= '%s'" % str(newchunks))
    return(newchunks)
...
def process_log(file_name):
    ...
    for i,line in enumerate(file_array):
        ...
        if mobj:
            ...
            log.debug("pre-cleaned rawdata - mobj.PERFDATA: '%s'" % mobj.PERFDATA)
            log.debug("Cleaning up perfdata string with parser_enhanced()...")
            mobj.PERFDATA = ' '.join(parser_enhanced(mobj.PERFDATA))
            log.debug("post-cleaned rawdata - mobj.PERFDATA: '%s'" % mobj.PERFDATA)
...
def process_spool_dir(directory):
    ...
    for perfdata_file in perfdata_files:
        ...
        log.debug('perfdata_file="%s", file_dir="%s"' % (str(perfdata_file),str(file_dir)))
        log.debug("Sending perfdata_file and file_dir to check_skip_file()")
        if check_skip_file(perfdata_file, file_dir):
            continue
        num_files += 1
        log.debug("Sending file_dir to process_log()...")
...
def check_skip_file(file_name, file_dir):
    ...
    """
    log.debug("Entering check_skip_file() with file_name='%s' and file_dir='%s'" % (str(file_name),str(file_dir)))
    if (
        file_name == "host-perfdata" or
        file_name == "service-perfdata"
    ):
        log.debug("If you see this debug msg it means file_name == 'host-perfdata' or 'service-perfdata'")
        return True
